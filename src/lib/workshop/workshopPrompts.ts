...
2. 读取剧本内容（docx/pdf 用相应 skill 或工具；纯文本直接 read）
   ...
   - 选取 3-12 条能覆盖主要人物、关系和事件的原文短句，逐字放入 workshop_set_breakdown.sourceEvidence；不要润色或概括
...
3. 深度理解剧本后，调用 workshop_set_breakdown 一次性写入：
   - sourceEvidence：3-12 条剧本原文短句，作为本次拆解的事实收据
...
5. 再调用 workshop_set_shots 写入分镜表（每镜：shotNo 用"集-镜"如 01-01、画面描述、对白、景别、运镜、情绪、时长 8-15 秒（Seedance 2.5 项目可到 30 秒）、关联 characterIds/sceneId）。**长剧本分批写，每批 ≤15 镜，第二批起用 mode:"merge"**
...
4. 调用 workshop_set_prompts 批量写入（每批 ≤15 条）：
   ...
   - imagePrompt（中文，gpt-image-2 生分镜图）：本镜首帧画面。
     **必须用 @图片N 引用场景和角色参考图**（编号规则同 videoPrompt：以 workshop_get_state 返回的 referenceOrder 为准；默认只有最终场景资产占用 @图片一；仅用户明确启用多角度参考时，多张场景图才会连续占用 @图片一/@图片二/@图片三…，角色/道具从后续编号开始）。
     ...
     必须写成导演可执行的首帧画面：空间锚点 + 主体站位 + 景别构图 + 表演瞬间 + 光线色温 + 材质细节 + 画风约束。不要只写剧情梗概。
