...
export function omniMgAgentGuide(): string {
  return [
    ...
    '7. 标准 MG 生成必须先做视觉预制：GPT-Image-2.5 先生成一张包含全部重复元素的母版概念图，再基于母版并行生成 2-4 张同风格关键帧，最后把母版、关键帧和用户原始素材一起交给 H3、Omni 或 Seedance Mini。母版负责锁定元素、色彩、材质和空间语言，派生帧只改变构图、景别和动作阶段。',
    ...
    '12. 用户反馈“不满意/文字还是错/有错字/乱码/字幕不对/字不对/文案不对/还是不行”时，不要继续反复调 Omni；剪辑视图调用 timeline_mg_text_fallback，画布/普通聊天调用 mg_text_fallback_generate：GPT-Image-2.5 先生成文字定版图，再用筷子 Seedance 2.0 Mini 图生视频。',
