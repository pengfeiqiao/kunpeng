...
test('gpt-image 引擎在存在对应模式路由时放行', () => {
  ...
  assert.equal(gptImageRouteExists(routes, 'gpt-image-2.5', false), true);
  assert.equal(gptImageRouteExists(routes, 'gpt-image-2.5', true), true);
  assert.equal(gptImageRouteExists(routes, 'gpt-image-2.5-i2i', true), true);
});

test('旧引擎 id gpt-image-2 作为别名同样放行（存量草稿/任务记录回归）', () => {
  const routes = [
    route({ id: 'api:slot1:standard:text-to-image', mode: 'text-to-image' }),
    route({ id: 'api:slot1:standard:image-to-image', mode: 'image-to-image' }),
  ];
...
test('没有对应模式路由时放行失败（文生请求不能拿图生路由充数）', () => {
  ...
  assert.equal(gptImageRouteExists(i2iOnly, 'gpt-image-2.5', false), false);
  assert.equal(gptImageRouteExists(i2iOnly, 'gpt-image-2.5', true), true);
});

test('空路由表 / 非 gpt-image 引擎一律不放行', () => {
  const routes = [route({ id: 'api:slot1:standard:text-to-image', mode: 'text-to-image' })];
  assert.equal(gptImageRouteExists([], 'gpt-image-2.5', false), false);
...
test('seedream 路由不算 gpt-image 渠道', () => {
  ...
  assert.equal(gptImageRouteExists(seedreamOnly, 'gpt-image-2.5', false), false);
});
