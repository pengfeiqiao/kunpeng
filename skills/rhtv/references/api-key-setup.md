...
Run `--check` first:
```bash
python3 ~/.kunpeng/skills/rhtv/scripts/runninghub.py --check
```

React by `status`:
- `"ready"` → "账号就绪！余额 ¥{balance}，想做点什么？生图、视频、配音都可以找我～"
- `"no_key"` → Guide: 1) 注册 runninghub.cn 2) 创建 Key 3) 充值 4) 发 Key 给我或在鲲鹏设置里配置
...
python3 -c "
import json, pathlib
p = pathlib.Path.home() / '.kunpeng' / 'kunpeng.json'
...
Replace `THE_KEY` with the actual key. Alternatively, the user can set `RUNNINGHUB_API_KEY` in their shell profile or in 鲲鹏 settings.
