...
python3 ~/.kunpeng/skills/rhtv/scripts/runninghub_app.py --list --sort RECOMMEND --size 10

# Hottest apps in the last 7 days
python3 ~/.kunpeng/skills/rhtv/scripts/runninghub_app.py --list --sort HOTTEST --size 10 --days 7

# Newest apps
python3 ~/.kunpeng/skills/rhtv/scripts/runninghub_app.py --list --sort NEWEST --size 10

# Page 2
python3 ~/.kunpeng/skills/rhtv/scripts/runninghub_app.py --list --sort RECOMMEND --size 10 --page 2
```
...
**Present apps to the user with cover images**. Display each app's cover image inline in your response:

```
For each app in the list:
  1. Show cover image inline with title and description
  2. Move to next app
After all apps:
...
Rules:
- ALWAYS display cover images inline — NEVER show cover URLs or file paths as text
...
```bash
python3 ~/.kunpeng/skills/rhtv/scripts/runninghub_app.py --info WEBAPP_ID
```
...
This is critical — AI app tasks are slow, and users need to know the task has started.

Map user's modifications to `--node` and `--file` arguments:

```bash
# Modify text node + upload image
python3 ~/.kunpeng/skills/rhtv/scripts/runninghub_app.py --run WEBAPP_ID \
  --node "52:prompt=make her hair into a short bob cut" \
  --file "39:image=/tmp/kunpeng/rh-output/photo.jpg" \
  -o /tmp/kunpeng/rh-output/app_result_$(date +%s).png
...
Same rules as standard API: display results inline, report cost, sync to canvas, suggest next steps.
...
python3 ~/.kunpeng/skills/rhtv/scripts/runninghub_app.py --info 1877265245566922800

# Run AI app with text modification
python3 ~/.kunpeng/skills/rhtv/scripts/runninghub_app.py --run 1877265245566922800 \
  --node "52:prompt=a boy with sunglasses" \
  -o /tmp/kunpeng/rh-output/app_$(date +%s).png
