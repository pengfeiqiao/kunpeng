#!/usr/bin/env python3
"""
RunningHub universal API client for Kunpeng.
...
def read_key_from_config() -> str | None:
    """Read API key from ~/.kunpeng/kunpeng.json"""
    cfg_path = Path.home() / ".kunpeng" / "kunpeng.json"
...
def require_api_key(provided_key: str | None) -> str:
    ...
    result = {
        ...
        "steps": [
            ...
            "4. Send the key in chat or set RUNNINGHUB_API_KEY environment variable",
        ],
    }
...
def read_cos_transit_endpoint() -> str | None:
    cfg_path = Path.home() / ".kunpeng" / "settings.json"
    if not cfg_path.exists():
        return None
    try:
        cfg = json.loads(cfg_path.read_text(encoding="utf-8"))
        ep = cfg.get("state", {}).get("cosTransitEndpoint", "")
...
def cos_transit_download(remote_url: str, file_name: str) -> str | None:
    """Call SCF cloud function to transit-download to COS, return COS URL."""
    endpoint = read_cos_transit_endpoint()
    if not endpoint:
        return None
    ...
    result = subprocess.run(cmd, capture_output=True, text=True)
    if result.returncode != 0:
        print(f"[COS transit] SCF request failed: {result.stderr[:200]}", file=sys.stderr)
        return None
    try:
        resp = json.loads(result.stdout)
        ...
        if cos_url:
            size = resp.get("size", "?")
            print(f"[COS transit] OK, size={size}, cosUrl={cos_url[:80]}...", file=sys.stderr)
            return cos_url
        print(f"[COS transit] no cosUrl in response: {result.stdout[:200]}", file=sys.stderr)
        return None
    except json.JSONDecodeError:
        print(f"[COS transit] invalid JSON: {result.stdout[:200]}", file=sys.stderr)
        return None
...
def download_file(url: str, output_path: str) -> str:
    ...
    if ext in (".mp4", ".mov", ".webm", ".avi"):
        file_name = f"transit_{int(time.time())}_{Path(output_path).name}"
        print(f"Attempting COS transit for video...", file=sys.stderr)
    ...
    if result.returncode != 0:
        if download_url != url:
            print(f"COS download failed, retrying direct...", file=sys.stderr)
...
def cmd_execute(args):
    ...
    if not output_path:
        ...
        output_path = f"/tmp/kunpeng/rh-output/result.{ext}"
