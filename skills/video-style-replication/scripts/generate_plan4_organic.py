...
def generate_organic_version(ref_image_path):
    """生成生物化实体版本"""
    ...
    prompt = """
【核心指令】
参考附件人物形象，完全复刻面部特征，生成一张震撼的单幅画面：女主角在雨崩看到悬浮的巨型转经筒矩阵，能量光束为生物化实体结构。
...
- 风格：未来废土风 + 轻微油画感（+15%）
- 转经筒：几何符号化（抽象）
- 能量光束：生物化实体（金属藤蔓+能量液体）
- 人物：复刻参考图，占比1-2%
- 场景：雨崩草甸+梅里雪山+经幡
- 构图：横向广角，低角度仰拍
"""

    output_path = Path.home() / "Desktop" / "视频复刻" / "方案4_生物化实体.jpg"
...
def main():
    # Usage: python3 generate_plan4_organic.py <参考图路径>
    if len(sys.argv) != 2:
        print("Usage: python3 generate_plan4_organic.py <参考图路径>")
        sys.exit(1)
    result = generate_organic_version(sys.argv[1])
    if result:
        print("\n" + "="*80)
        print("【方案4生成完成】")
        print("="*80)
        print("特点：生物化实体能量光束（金属藤蔓+能量液体）")
